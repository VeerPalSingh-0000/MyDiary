...
import { LogOut, Book, Star, Settings, ChevronRight, PenLine, X, Trash2 } from "lucide-react";
...
export default function Sidebar({ 
  ...
  onEntryClick = () => {},
  onViewAll = () => {},
  onDelete = () => {} // 1. Accept onDelete prop
}) {
        ...
        <div className="relative z-10 flex flex-col h-full">
          <div className="p-6 md:p-8 pb-6 shrink-0">
            ...
            <div className="flex items-center gap-4 group cursor-default">
              <div className="relative shrink-0">
                {user?.photoURL ? (
                  <img src={user.photoURL} alt="Profile" className="w-12 h-12 rounded-2xl object-cover shadow-lg ring-2 ring-white/10" />
                ) : (
                  <div className="w-12 h-12 rounded-2xl bg-gradient-to-br from-indigo-500 to-purple-600 text-white flex items-center justify-center font-bold text-lg shadow-lg">
                    {getUserInitials()}
                  </div>
                )}
              ...
              <div className="overflow-hidden">
                <h3 className="text-base font-bold text-white truncate">{user?.displayName || "Writer"}</h3>
                <p className="text-xs text-slate-400 font-medium truncate">{user?.email || "Signed in"}</p>
              </div>
            </div>
          </div>

          <nav className="px-4 py-2 shrink-0">
            <p className="px-4 text-[10px] font-bold text-slate-500 uppercase tracking-widest mb-2 select-none">Menu</p>
                ...
                return (
                  <button
                    ...
                    className={`w-full flex items-center gap-3 px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 group relative overflow-hidden ${
                      isActive ? "bg-indigo-500/20 text-indigo-300 shadow-sm ring-1 ring-indigo-500/30" : "text-slate-400 hover:bg-white/5 hover:text-white"
                    }`}
                  >
          ...
          <div className="flex-1 px-4 py-6 overflow-y-auto custom-scrollbar min-h-0 flex flex-col">
            <div className="flex items-center justify-between px-4 mb-3 shrink-0">
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest select-none">Recent</p>
              {entries.length > 0 && (
                <button onClick={onViewAll} className="text-[10px] font-bold text-indigo-400 hover:text-indigo-300 hover:bg-indigo-500/10 px-2 py-1 rounded transition-all">VIEW ALL</button>
              )}
            </div>
              ...
              {entries.length === 0 ? (
                ...
                entries.map((entry) => (
                  <div
                    ...
                    className="w-full text-left group flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 border border-transparent hover:border-white/5 transition-all duration-200 cursor-pointer"
                  >
                    <div className="min-w-[24px] h-6 flex items-center justify-center bg-white/5 rounded-md text-sm group-hover:bg-indigo-500/20 transition-colors">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-slate-500 mt-0.5">{formatDate(entry.date || entry.createdAt)}</p>
                    </div>

                    {/* 2. Delete Button (Visible on Hover) */}
                    <button
                        onClick={(e) => {
                            e.stopPropagation(); // Stop opening the entry
          ...
          <div className="p-4 border-t border-white/10 bg-[#050505]/50 shrink-0">
            <button onClick={logout} className="w-full flex items-center justify-center gap-2 px-4 py-3 text-sm font-medium text-slate-400 hover:text-rose-400 hover:bg-rose-500/10 rounded-xl transition-all duration-200 group">
